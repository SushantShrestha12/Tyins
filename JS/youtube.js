...
async function startDownload() {
  try {
      ...
      const response = await fetch(`https://localhost:7168/Youtube/download?url=${encodeURIComponent(url)}`);

      if (!response.ok) {
          document.getElementById('message').textContent = 'Failed to download video. Response status: ' + response.status;
          return;
      }

      const contentLength = response.headers.get("Content-Length");
      if (!contentLength) {
          document.getElementById('message').textContent = 'Content-Length header is missing';
          return;
      }
      ...
      document.getElementById('message').textContent = 'Error downloading video: ' + error.message;
  }
}
